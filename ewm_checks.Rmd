...
le_tester <- function(df, later_tstp, earlier_tstp) {
  ...
  a <- df %>%
    mutate_(result = paste(later_tstp_name,
                           "-",
                           earlier_tstp_name,
                           sep = "")) %>%
    # mutate_(result = time_length((later_tstp_name - earlier_tstp_name),
    #                             "seconds") - result_name_abap) %>%
    filter(result != 0)
...
le_tester(df = dfx,
          ...
          earlier_tstp = 20)
a <- dfx %>%
  mutate_(result = time_length(("GEWTSTP30" - "GEWTSTP20"),
                              "seconds") - "GEW2030L") %>%
  filter(result != 0)
